{
  ...
  inputs.byond = {
    ...
    url = "https://www.byond.com/download/build/515/515.1616_byond.zip";
    flake = false;
  };

  outputs = { self, nixpkgs, byond, ... }: let byond_ver = "515"; byond_build = "1616"; in rec {
    packages.x86_64-linux.byond = with import nixpkgs { config.allowUnfree = true; system = "x86_64-linux"; };
      stdenv.mkDerivation (let wineprefix = "~/.wineprefix/byond"; in rec {
        ...
        installPhase = ''
          ...
          echo "${winetricks}/bin/winetricks mimeassoc=off allfonts mfc42 gdiplus vcrun2022 wsh57 windowscodecs ogg ole32 riched30 wmp10 vlc" >> runbyond
